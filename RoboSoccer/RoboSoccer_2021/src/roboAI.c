...
double self_angle;
int go_buffer = 0;
int go_buffer_flag = 0;
int kick_buffer = 230;
int kick_ready = 0;
int kicking = 0;
double pre_dist = 2000;
double onetx = 0;
double onety = 0;
double snetx = 0;
double snety = 0;
...
//  fprintf(stderr,"Just trackin'!\n");	// bot, opponent, and ball.
  ...
  printf("ball opp dist: %f\n", dist(ball_x, ball_y, ai->st.old_ocx, ai->st.old_ocy));
  // printf("nx: %f ny: %f \n", onetx, onety);
  // get_away_from_location(ai, ball_x, ball_y, 300, 1);
  // return;

  if (ai->st.state >= 1 && ai->st.state <= 99) {
    if (ai->st.state == 1) {//dispatch
      if (close_to_opp(ai)){//close to opponent
        ai->st.state = 20;
        return;
      } else if (opp_close_to_ball(ai)) {//opponent get the ball
        ai->st.state = 30;
      } else if (at_boundary(ball_x, ball_y, 1.5*SELF_WIDTH)) {//ball ay boundary
        ai->st.state = 2;
      } else {//ball at middle
        ai->st.state = 10;
      }
    } else if (ai->st.state == 2){//ball ay boundary
      if (close_to_opp(ai)){
        ai->st.state = 20;
        return;
      } else if (opp_close_to_ball(ai)) {//opponent get the ball
        ai->st.state = 30;
      } else if(!at_boundary(ball_x, ball_y, 1.5*SELF_WIDTH)){//ball at middle
        ai->st.state = 10;
      }
      go_to_location(ai, ball_x + ballm_x, ball_y + ballm_y, 3, 120);
      return;
      ...
      if (close_to_opp(ai)){
       ...
       go_util_close(ai, ball_x, ball_y, 4, 100, 5);
       return;
      ...
      recover_kick(ai, 1);
      return;     
    } else if (ai->st.state == 10) {//ball at middle
      if (compare_by_side(ai, ball_x, ai->st.old_scx)) {//ball on our side
        ai->st.state = 11;
        return;
      } else {//we at opp side
        double dist = BALL_WIDTH/2 + SELF_WIDTH;

        if (fabs(ball_y-ai->st.old_scy) < dist) {
          ai->st.state = 16;
        } else { 
          ai->st.state = 17;
        }
      }
    } else if (ai->st.state == 11) {//we toward the ball and goal
      if (close_to_opp(ai)) {
        ai->st.state = 20;
        return;
      } else if (opp_close_to_ball(ai)) {//opponent get the ball
        ai->st.state = 30;
      } else if(at_boundary(ball_x, ball_y, 1.5*SELF_WIDTH)){
        ai->st.state = 2;
      }
      go_behind_ball(ai, 12);
    } else if (ai->st.state == 12) {
      face_to(ai, onetx, onety, 13, 0);
    } else if (ai->st.state == 13) {
      go_util_close(ai, ball_x, ball_y, 14, 100, 4);
    } else if (ai->st.state == 14) {
      heavy_kick(ai, 15);
    } else if (ai->st.state == 15) {
      recover_kick(ai, 1);    
    } else if (ai->st.state == 16) {
      if (close_to_opp(ai)) {
        ai->st.state = 20;
        return;
      } else if (opp_close_to_ball(ai)) {//opponent get the ball
        ...
        go_away_from_ball(ai, 17);
      }
    } else if (ai->st.state == 17) {
      if (close_to_opp(ai)) {
        ai->st.state = 20;
        return;
      } else if (opp_close_to_ball(ai)) {//opponent get the ball
    ...
    } else if (ai->st.state == 20) {//close to opponent
      if (!close_to_opp(ai)){
        ai->st.state = 1;
        return;
      }
      if (at_boundary(ai->st.old_scx, ai->st.old_scy, SELF_WIDTH)) {
        BT_all_stop(0);
        return;
      } else {
        get_away_from_location(ai, ai->st.old_ocx, ai->st.old_ocy, 250, 1);
        return;
      }
    } else if (ai->st.state == 30) {
      if (space_between_opp_goal(ai)) {
        ai->st.state = 31;
      } else {
        BT_all_stop(0);
        ai->st.state = 1;
      }
    } else if (ai->st.state == 31) {//go to net
      if (close_to_opp(ai)) {
        ai->st.state = 20;
      }else if(!opp_close_to_ball(ai)){
        ai->st.state = 1;
      } else{
        go_to_self_net(ai, 32);
      }
    } else if (ai->st.state == 32) {//go toward ball
      if(!opp_close_to_ball(ai)){
        ai->st.state = 1;
      } else if(close_to_opp(ai)){
        BT_all_stop(1);
      } else {
        printf("here\n");
        go_to_location(ai, ball_x + ballm_x, ball_y + ballm_y, 32, 250);
      }
    }
    ...
    // } else
     if (ai->st.state == 101) {
      ...
      face_to(ai, onetx, onety, 103, 0);

    } else if (ai->st.state == 103) {
      go_util_close(ai, ball_x, ball_y, 104, 50, 4);
      return;
    ...
    if (ai->st.state == 201) {
      ...
      go_to_location(ai, ball_x + ballm_x, ball_y + ballm_y, 202, 120);
      return;
...
int space_between_opp_goal(struct RoboAI *ai) {
  ...
  if (snetx == 1050) {
    v1 = snetx - 2*defense_zone;

    return ai->st.old_ocx < v1 || fabs(ai->st.old_scy-snety) > defense_zone;
  } else {
    v1 = snetx + 2*defense_zone;
...
void get_away_from_location(struct RoboAI *ai, int x, int y, int distance, int back_to_state) {
  double sox = x - ai->st.old_scx;
  double soy = y - ai->st.old_scy;
  double target_a = get_angle_from_vector(sox, soy);
  double error = target_a - self_angle;
  int power = 20;
  ...
  if (fabs(error) > 90) {
    target_a += 180;
    target_a = fmod(target_a, 360);
  }

  double new_error = target_a - self_angle;
  double d = dist(ai->st.old_scx, ai->st.old_scy, x, y);

  if (d > distance) {
    ai->st.state = back_to_state;
    return;
  } else if (fabs(new_error) > 40) {
    if(new_error>0){
    ...
    if (fabs(error) > 90) {
      BT_drive(MOTOR_B, MOTOR_C, power);
    } else {
      BT_drive(MOTOR_B, MOTOR_C, -power);
    }
  }
...
void go_away_from_ball(struct RoboAI *ai, int back_to_state) {
   if (fabs(ball_y-ai->st.old_scy) < BALL_WIDTH/2 + SELF_WIDTH) {
      if (ball_y < 360) {
        go_to_location(ai, ai->st.old_scx, 720, back_to_state, 10);
      } else {
        go_to_location(ai, ai->st.old_scx, 0, back_to_state, 10);
      }
    } else {
      ai->st.state = back_to_state;
    }
...
void go_to_side(struct RoboAI *ai, int back_to_state) {
    ...
    double targety = ball_y;
    
    double move_dist = 2*SELF_WIDTH;

    if (ball_y > ai->st.old_scy) {
      //go up
      targety -= move_dist;
      printf("%f %f\n", targetx, targety);
      go_to_location(ai, targetx, targety, back_to_state, 50);
    } else {
      //go down
      targety += move_dist;
      printf("%f %f\n", targetx, targety);
...
void go_to_self_net(struct RoboAI *ai, int back_to_state) {
  ...
  if (snetx == 1050) {
    x = snetx - defense_zone;
  } else {
    x = snetx + defense_zone;
  }

  go_to_location(ai, x, y, back_to_state, 50);
}
...
void go_to_location(struct RoboAI *ai, double x, double y, int back_to_state, int threshold) {
  double error = get_angle_from_vector(x-ai->st.old_scx, y-ai->st.old_scy) - self_angle;
  if (error < -180) {
    error += 360;
  } else if (error > 180) {
  ...
  //close to ball
  if (d < threshold && error < 8 && error > -8) {
    BT_all_stop(1);
...
void go_util_close(struct RoboAI *ai, double x, double y, int back_to_state, int threshold, int buffer) {
  ...
  if (d > threshold && go_buffer == 0) {
    BT_all_stop(1);
    BT_drive(MOTOR_B, MOTOR_C, 50);
    return;
  //not facing ball 
  } else if(go_buffer > 0){
    if(buffer != 4 || go_buffer > 2){
      BT_all_stop(1);
    }
    BT_drive(MOTOR_B, MOTOR_C, 50);
    go_buffer -= 1;
    return;
  }else if (d < threshold && go_buffer_flag == 0) {
    go_buffer = buffer;
    go_buffer_flag = 1;
    if(buffer != 4){
      BT_all_stop(1);
...
int close_to_opp(struct RoboAI *ai) {
  return dist(ai->st.old_scx, ai->st.old_scy, ai->st.old_ocx, ai->st.old_ocy) < 230;
}

int opp_close_to_ball(struct RoboAI *ai) {
  return dist(ai->st.old_ocx, ai->st.old_ocy, ball_x, ball_y) < 150;
}

int at_boundary(double x, double y, double d) {
  return fabs(x-0) < d || fabs(x-1024) < d || fabs(y-0) < d || fabs(y-720) < d;
}